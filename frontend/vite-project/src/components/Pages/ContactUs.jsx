...
const ContactUs = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        const TO_EMAIL = import.meta.env.VITE_EMAILJS_TO_EMAIL || 'ankitshaw6999@gmail.com'
        ...
        try {
            ...
            const errText = err?.text || err?.message || 'Failed to send message. Please try again later.'
            setStatusMsg({ type: 'error', text: `Failed to send message: ${errText}` })
        } finally {
            setSending(false)
        ...
        <div className="bg-gray-50 pt-16 sm:pt-20 pb-6 px-4 sm:px-6 lg:px-8">
            <div className="max-w-6xl mx-auto">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 sm:gap-7">
                    {/* Contact Form */}
                    <form onSubmit={handleSubmit} className="bg-white rounded-2xl sm:rounded-3xl shadow-xl p-5 sm:p-7 space-y-5 sm:space-y-6 border border-gray-100 order-2 lg:order-1">
                        {statusMsg && (
                            <div className={`rounded-md px-4 py-2 text-sm font-medium ${statusMsg.type === 'error' ? 'bg-red-100 text-red-800' : 'bg-emerald-50 text-emerald-800'}`}>
                                {statusMsg.text}
                            </div>
                        )}
                        <div>
                            ...
                            <input
                                ...
                                readOnly={!!user}
                                className={`w-full px-3 sm:px-4 py-2.5 sm:py-3 border border-gray-300 rounded-lg sm:rounded-xl outline-none focus:ring-2 focus:ring-emerald-500 transition text-sm sm:text-base ${user ? 'bg-gray-100 cursor-not-allowed' : ''}`}
                            />
                        </div>
                        ...
                        <button
                            type="submit"
                            disabled={sending}
                            className={`w-full text-sm sm:text-base px-6 sm:px-8 py-2.5 sm:py-3 rounded-lg sm:rounded-xl shadow-lg font-semibold flex items-center justify-center ${sending ? 'bg-emerald-400 text-white cursor-not-allowed' : 'bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-700 hover:to-emerald-800 text-white'}`}
                        >
                        ...
                        <div className="space-y-4 sm:space-y-5">
                            <div className="flex items-center gap-3 sm:gap-4 p-3 sm:p-4 bg-gray-50 rounded-lg sm:rounded-xl">
                                ...
                                <div className="min-w-0 flex-1">
                                    ...
                                    <p className="text-gray-600 text-xs sm:text-sm truncate">ankitshaw6999@gmail.com</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-3 sm:gap-4 p-3 sm:p-4 bg-gray-50 rounded-lg sm:rounded-xl">
                                <div className="bg-emerald-100 text-emerald-600 rounded-full p-2 flex-shrink-0">
